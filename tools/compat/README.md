...
| `errorlog_report.py` | Normalizes `error.log` into a (subsystem, signature, count) table. `--diff base.log new.log` prints **net-new signatures** — the exit gate for every migration phase (exit code 1 if any). `--noise vanilla.log` subtracts signatures that a no-mods vanilla run also produces (see `COMPAT-AUDIT…/09-vanilla-noise-4.4.6-error.log`). |
